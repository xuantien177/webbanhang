...
<div class="container">
    <div class="row">
        <div class="col-md-4">
            <?=$form->field($donHang,'ho_ten')->label('Họ tên') ?>
        </div>
        <div class="col-md-4">
            <?=$form->field($donHang,'dien_thoai')->label('Điện thoại') ?>
        </div>
        <div class="col-md-4">
            <?=$form->field($donHang,'email')->label('Email') ?>
        </div>
    </div>
    <?=$form->field($donHang,'dia_chi_giao_hang')->label('Địa chỉ giao hàng') ?>
    <?=$form->field($donHang,'ghi_chu')->textarea()->label('Ghi chú') ?>
</div>
...
<div class="container">
    <h3>ĐƠN HÀNG CỦA BẠN</h3>
        ...
        <div class="region-action">
            <?=\yii\helpers\Html::submitInput('Lưu lại', ['class'=>'btn btn-sm btn-primary']) ?>
        </div>
    </div>
